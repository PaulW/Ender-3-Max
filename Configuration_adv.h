...
#define CHAMBER_HYSTERESIS \
  2  // (°C) Only set the relevant heater state when ABS(T-target) > CHAMBER_HYSTERESIS
#endif
#endif
...
#define CHAMBER_FAN_MODE \
  2  // Fan control mode: 0=Static; 1=Linear increase when temp is higher than target; 2=V-shaped curve; 3=similar to 1 but fan is always on.
...
#define COOLER_FAN_BASE \
  100  // Base Cooler fan PWM (0-255); turns on when Cooler temperature is above the target
...
#if ANY(THERMAL_PROTECTION_HOTENDS, THERMAL_PROTECTION_BED, THERMAL_PROTECTION_CHAMBER, \
...
#define X1_MAX_POS \
  X_BED_SIZE            // A max coordinate so the X1 carriage can't hit the parked X2 carriage
...
#define SHAPING_ZETA_X \
  0.15f  // Damping ratio of the X axis (range: 0.0 = no damping to 1.0 = critical damping).
#endif
#if ENABLED(INPUT_SHAPING_Y)
#define SHAPING_FREQ_Y 40  // (Hz) The default dominant resonant frequency on the Y axis.
#define SHAPING_ZETA_Y \
  0.15f  // Damping ratio of the Y axis (range: 0.0 = no damping to 1.0 = critical damping).
#endif
...
#define DEFAULT_MINIMUMFEEDRATE \
  0.0  // (mm/s. °/s for rotational-only moves) Minimum feedrate. Set with M205 S.
#define DEFAULT_MINTRAVELFEEDRATE \
  0.0  // (mm/s. °/s for rotational-only moves) Minimum travel feedrate. Set with M205 T.
...
#define PLR_ENABLED_DEFAULT \
  true  // Power Loss Recovery enabled by default. (Set with 'M413 Sn' & M500)
...
#define SDSORT_DYNAMIC_RAM \
  true  // Use dynamic allocation (within SD menus). Least expensive option. Set SDSORT_LIMIT before use!
...
#define TOOLCHANGE_FS_WIPE_RETRACT \
  0  // (mm) Cutting retraction out of park, for less stringing, better wipe, etc. Adjust with LCD or M217 G.
...
#define FILAMENT_CHANGE_UNLOAD_FEEDRATE \
  200  // (mm/s) Unload filament feedrate. This can be pretty fast.
#define FILAMENT_CHANGE_UNLOAD_ACCEL 25  // (mm/s^2) Lower acceleration may allow a faster feedrate.
#define FILAMENT_CHANGE_UNLOAD_LENGTH                           \
  460  // (mm) The length of filament for a complete unload.    \
...
#define FILAMENT_CHANGE_FAST_LOAD_FEEDRATE \
  75  // (mm/s) Load filament feedrate. This can be pretty fast.
...
#define FILAMENT_CHANGE_FAST_LOAD_LENGTH                              \
  410  // (mm) Load length of filament, from extruder gear to nozzle. \
       //   For Bowden, the full length of the tube and nozzle.       \
       //   For direct drive, the full length of the nozzle.
...
#define ADVANCED_PAUSE_PURGE_LENGTH                                          \
  25  // (mm) Length to extrude after loading.                               \
...
#define SPINDLE_LASER_FREQUENCY                                                             \
  2500  // (Hz) Spindle/laser frequency (only on supported HALs: AVR, ESP32, and LPC)       \
...
#define MAX7219_DEBUG_PROFILE                                                          \
  6   // Display the fraction of CPU time spent in profiled code on this LED matrix    \
